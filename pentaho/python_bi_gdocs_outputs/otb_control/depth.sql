WITH collections as (
	SELECT
	CASE WHEN MONTH(%(date_to)s) BETWEEN 2 and 7 THEN 'OI' ELSE 'V' END +
	CAST(CASE WHEN MONTH(%(date_to)s) BETWEEN 2 and 7 THEN YEAR(%(date_to)s) ELSE YEAR(%(date_to)s)+1 END AS CHAR) as collection
	UNION
	SELECT
	CASE WHEN MONTH(%(date_to)s) BETWEEN 2 and 7 THEN 'OI' ELSE 'V' END +
	CAST(CASE WHEN MONTH(%(date_to)s) BETWEEN 2 and 7 THEN YEAR(%(date_to)s)-1 ELSE YEAR(%(date_to)s) END AS CHAR) as collection
	UNION
	SELECT
	CASE WHEN MONTH(%(date_to)s) BETWEEN 2 and 7 THEN 'V' ELSE 'OI' END +
	CAST(YEAR(%(date_to)s) AS CHAR) as collection
),
base as (
	SELECT
	ppc.produto,
